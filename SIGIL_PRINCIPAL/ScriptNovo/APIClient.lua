...
local PLANTA_UUID = "661e8415-65eb-4821-86e9-462d7ad57c9e" -- TODO: Definir UUID real da planta

-- Token da API da planta (deve ser configurado)
-- Token da planta SIGIL (661e8415-...). Validado via GET .../motores -> HTTP 200 em 2026-05-19.
-- ATENÇÃO: trocar este token exige recompilar e baixar o projeto na IHM de novo.
local PLANTA_API_TOKEN = "piJ6FdgWGUCGdV-AruWLcQ-EG7dqaX4CES5WhwPdbGtWQ"
...
function APIClient:httpRequest(method, endpoint, data)
    ...
    if method == "GET" then
        ...
        if response_body_table and #response_body_table > 0 then
            ...
            -- ltn12.sink.table guarda a resposta em VÁRIOS chunks (~2KB cada).
            -- Pegar só [1] truncava o JSON em respostas grandes (>2KB) -> json.decode falhava.
            -- Concatenar TODOS os pedaços reconstrói o body completo.
            response_body = table.concat(response_body_table)
        else
